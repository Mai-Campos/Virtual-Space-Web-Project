...
function DirectorManagement() {
  ...
  const [editingDirectorId, setEditingDirectorId] = useState<number | null>(
    null,
  );
  ...
  const validateForm = () => {
    ...
    if (!directorName || directorName.trim().length < 4) {
  ...
  useEffect(() => {
    const fetchDirectors = async () => {
      try {
        ...
        if (!res.ok) {
          throw new Error("Error cargando directores");
        }
        ...
        toast.error("Error cargando directores");
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editingDirectorId
      ? `http://localhost:3000/api/v1/directors/${editingDirectorId}`
      : `http://localhost:3000/api/v1/directors`;

    const method = editingDirectorId ? "PATCH" : "POST";
    ...
    toast.success("Director guardado correctamente");
  ...
  const handleDelete = async (id: number) => {
    const confirmDelete = confirm("¿Seguro que deseas eliminar este director?");
    if (!confirmDelete) return;

    try {
      const res = await fetch(`http://localhost:3000/api/v1/directors/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      if (!res.ok) {
        ...
        if (errorBody.statusCode === 409) {
          throw new Error(
            "No se puede eliminar el director porque está asociado a una o más películas.",
          );
        }
  ...
  const handleEdit = (director: Options) => {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
      <div className="col-span-1 bg-white/5 rounded-lg p-6">
        <h2 className="text-white text-[22px] font-bold leading-tight tracking-[-0.015em] pb-6">
          {editingDirectorId ? "Editar Director" : "Añadir Nuevo Director"}{" "}
        </h2>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-400/80">
                  Editando director #{editingDirectorId}
                </p>
              </div>
            </div>
            <button
